...
int main(int argc, char *argv[])
{
	...
	char gets[30];
	msg_cola msg;
	struct st_data* segmento;
	id = shmget(CLAVE,1024,0666|IPC_CREAT);
	segmento = shmat(id,NULL,0);
	...
	int size = sizeof(msg.tipo)+sizeof(msg.datos);
	int val;
	semid = semget(CLAVE,MAX_SEM,0666|IPC_CREAT);
	int semval;
	int r;
	char speed[BUFLEN];
	...
	printf("%d\n",segmento[2].state);
	printf("%d\n",segmento[3].state);
	while(msgrcv(cola,(struct msgbuf *)&msg,size,canal,0))
	{
		i = 0;
		while(i<4){
			i++;
			if(strcmp(segmento[i].name,argv[1])==0)
				break;
			printf("Name -> %s\n",segmento[i].name);
		}
		printf("Recibido -> %c\n",msg.tipo);
		if(msg.tipo == '2'){
			sscanf(msg.datos,"<%s>",speed);
			segmento[0].speed = atoi(speed);
			segmento[1].speed = atoi(speed);
			segmento[2].speed = atoi(speed);
			segmento[3].speed = atoi(speed);
			sprintf(buffer,"%s %s",udp_cmd[2],speed);
		}
		else if(msg.tipo == '3'){
			val = *((int *) msg.datos);
			segmento[0].rpm = val;
			...
			segmento[2].rpm = val;
			segmento[3].rpm = val;
			sprintf(buffer,"%s %d",udp_cmd[3],val);
		}
		else if(msg.tipo == '4'){
			val = *((int *) msg.datos);
			semval = semctl(semid,val,GETVAL,0);
			sprintf(buffer,"%s %d %d",udp_cmd[4],val,semval);
		}
		else if(msg.tipo == '5'){
			sendto(s,"BYE",strlen("BYE"),0,(struct sockaddr *)&si_other,slen);
		}
		printf("Enviando -> %s\n",buffer);
		sendto(s,buffer,strlen(buffer),0,(struct sockaddr *)&si_other,slen);
